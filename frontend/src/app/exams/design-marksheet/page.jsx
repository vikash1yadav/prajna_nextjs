...
export default function DesignMarksheetPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/exams/templates/marksheet/${id}`, { method: 'DELETE' });
      const json = await res.json();
      if (json.success) {
  ...
  return (
    <div className="flex flex-col gap-8 pb-12">
      <div>
        <h2 className="text-3xl font-bold tracking-tight mb-1 text-white">Marksheet Layout Designer</h2>
        <p className="text-zinc-400 text-sm">Design templates, logos, school address metadata, signature placements, and togglable student parameters for transcripts.</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="glass-card h-fit flex flex-col gap-6">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div>
              ...
              <input 
                type="text" 
                placeholder="e.g. CBSE Grade Card 2026"
            ...
            <div>
              <label className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider block mb-1.5">School Address</label>
              <input 
                type="text" 
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input 
                  type="text" 
                  placeholder="e.g. GRADE SHEET TRANSCRIPT"
                  className="input-field w-full"
                  value={form.title}
            ...
            <div className="border-t border-white/5 pt-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { key: 'is_division', label: 'Division/Grade Result' }
                ].map(item => (
        ...
        <div className="glass-card xl:col-span-2 flex flex-col gap-6">
          <h3 className="text-lg font-semibold text-white">Marksheet Templates</h3>
      ...
      {previewTemplate && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white text-black w-full max-w-3xl rounded-2xl p-8 flex flex-col gap-6 shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <div className="border-[3px] border-zinc-900 p-6 rounded-xl relative flex flex-col gap-4">
              <div className="flex items-center justify-between border-b-2 border-zinc-200 pb-3">
                ...
                <div className="text-center flex-grow">
                  ...
                  <p className="text-[10px] text-zinc-500 font-semibold">{previewTemplate.school_address || 'School Address'}</p>
                  <h3 className="text-sm font-semibold uppercase mt-1">{previewTemplate.heading || 'REPORT CARD'}</h3>
                </div>
                {previewTemplate.right_logo && <img src={previewTemplate.right_logo} className="w-16 h-16 object-contain" />}
              </div>

              <div className="grid grid-cols-3 gap-4 text-xs font-semibold text-zinc-700 border-b border-zinc-200 pb-3">
                ...
                {previewTemplate.is_photo === 1 && (
                  <div className="w-20 h-24 border border-zinc-400 flex items-center justify-center text-[10px] text-zinc-400 bg-zinc-50 ml-auto rounded">
                    Student Photo
                  </div>
                )}
              </div>

              {/* Sample mark details table */}
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-zinc-100 border-b border-zinc-300">
                    <th className="p-2 font-bold">Subject</th>
                    <th className="p-2 font-bold">Max Marks</th>
                    <th className="p-2 font-bold">Min Marks</th>
                    <th className="p-2 font-bold">Marks Obtained</th>
                ...
                <tbody>
                  <tr className="border-b border-zinc-200">
                    ...
                    <td className="p-2 font-bold">85.00</td>
                    <td className="p-2 text-emerald-600 font-semibold">Pass</td>
                  </tr>
                  <tr className="border-b border-zinc-200">
                    <td className="p-2 font-semibold">Science</td>
                    <td className="p-2">100</td>
                    <td className="p-2">33</td>
                    <td className="p-2 font-bold">78.00</td>
                    <td className="p-2 text-emerald-600 font-semibold">Pass</td>
                  </tr>
                </tbody>
              </table>

              {previewTemplate.is_division === 1 && (
                <div className="text-xs font-bold text-zinc-900 mt-2 bg-zinc-100 p-2.5 rounded border border-zinc-200 flex justify-between">
                  <span>Grand Total: 163 / 200</span>
                  <span>Percentage: 81.50%</span>
                  <span>Result: FIRST DIVISION (PASS)</span>
                </div>
              )}
