...
import { Inbox, Plus, Edit2, Trash2, RotateCcw, Eye, X } from 'lucide-react';

const fetcher = (...args) => fetch(...args).then((res) => res.json());

export default function PostalReceivePage() {
  const { data: receiveRes, isLoading } = useSWR('/api/dispatch-receives?type=receive', fetcher);
  const items = receiveRes?.data || [];
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!fromTitle.trim()) {
      setStatusMessage({ type: 'error', text: 'Sender title is required' });
      return;
    }
    ...
    const payload = {
      ...
      to_title: toTitle.trim() || null,
      address: address.trim() || null,
      from_title: fromTitle.trim(),
      date: date || null,
      note: note.trim() || null,
      type: 'receive'
    };
    ...
    try {
      ...
      if (data.success) {
        setStatusMessage({
          type: 'success',
          text: editId ? 'Postal receipt updated successfully!' : 'Postal receipt recorded successfully!'
        });
        handleReset();
        mutate('/api/dispatch-receives?type=receive');
      } else {
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        <h2 className="text-3xl font-bold tracking-tight mb-1 text-indigo-500 flex items-center gap-2">
          <Inbox size={28} /> Postal Receive
        </h2>
        <p className="text-zinc-400 text-sm">Log and track incoming letters, documents, parcels, and target departments.</p>
      </div>
        ...
        <div className="col-span-1 lg:col-span-4">
          <div className="glass-card flex flex-col gap-4 border-indigo-500/10">
            <h3 className="text-lg font-semibold text-white">
              {editId ? 'Edit Receive Record' : 'Record Incoming Mail'}
            </h3>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="form-group flex flex-col gap-1">
                ...
                <input type="text" className="input-field" placeholder="Name or title of sender" value={fromTitle} onChange={(e) => setFromTitle(e.target.value)} disabled={isSubmitting} />
              </div>
              ...
              <div className="form-group flex flex-col gap-1">
                ...
                <input type="text" className="input-field" placeholder="Recipient department or official name" value={toTitle} onChange={(e) => setToTitle(e.target.value)} disabled={isSubmitting} />
              </div>
              ...
              <div className="form-group flex flex-col gap-1">
                ...
                <input type="text" className="input-field" placeholder="Sender address or origin details" value={address} onChange={(e) => setAddress(e.target.value)} disabled={isSubmitting} />
              </div>

              <div className="form-group flex flex-col gap-1">
                <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Note</label>
                <textarea className="input-field min-h-[80px] resize-none py-1.5" placeholder="Description of package content..." value={note} onChange={(e) => setNote(e.target.value)} disabled={isSubmitting} />
              </div>
        ...
        <div className="col-span-1 lg:col-span-8">
          <div className="glass-card !p-0 overflow-hidden border-indigo-500/10">
            <div className="p-5 border-b border-white/5 flex justify-between items-center">
              <h3 className="text-lg font-semibold text-white">Receive Registry</h3>
            ...
            {isLoading ? (
              <div className="p-12 text-center text-zinc-400 animate-pulse">Loading receive records...</div>
            ) : items.length > 0 ? (
                  ...
                  <tbody>
                    {items.map((item) => (
                      <tr key={item.id} className={`hover:bg-white/5 transition-colors ${editId === item.id ? 'bg-indigo-500/5' : ''}`}>
                        <td className="font-semibold text-white">{item.from_title}</td>
                        <td className="text-zinc-300 font-mono text-sm">{item.reference_no || '-'}</td>
                        <td className="text-zinc-400 text-sm">{item.to_title || '-'}</td>
              ...
              <div className="p-12 text-center text-zinc-500">No receive records found. Record one on the left.</div>
            )}
          </div>
        </div>
      </div>
      ...
      {selectedItem && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card max-w-xl w-full border-indigo-500/20 flex flex-col gap-6 relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div>
              <h3 className="text-2xl font-bold text-white">{selectedItem.from_title}</h3>
              <p className="text-sm text-zinc-400 font-mono">Postal Receive ID: #{selectedItem.id}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm border-t border-white/5 pt-4">
              ...
              <div>
                <span className="text-zinc-500 block text-xs font-semibold uppercase tracking-wider">Recipient Title</span>
                <span className="text-zinc-200">{selectedItem.to_title || '-'}</span>
              </div>
              <div>
              ...
              <div>
                <span className="text-zinc-500 block text-xs font-semibold uppercase tracking-wider">Origin Address</span>
                <span className="text-zinc-200">{selectedItem.address || '-'}</span>
              </div>
            </div>

            <div className="flex flex-col gap-1.5 border-t border-white/5 pt-4">
              <span className="text-zinc-500 text-xs font-semibold uppercase tracking-wider">Receive Notes</span>
