...
export default function IncomeHeadPage() {
  const { data: headsRes, isLoading } = useSWR('/api/income/heads', fetcher);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setStatusMessage({
          type: 'success',
          text: 'Income Head added successfully!'
        });
        handleReset();
        mutate('/api/income/heads');
      } else {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Are you sure you want to delete this category head? Any income items under it may be unlinked.')) {
      return;
    }
    try {
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        <h2 className="text-3xl font-bold tracking-tight mb-1 text-emerald-400 flex items-center gap-2">
          <TrendingUp size={28} /> Income Heads
        </h2>
        <p className="text-zinc-400 text-sm">Define and organize categories of miscellaneous incomes, like Donations, Rent, Fees, or Sales.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="col-span-1 lg:col-span-4">
          <div className="glass-card flex flex-col gap-4 border-emerald-500/10">
            <h3 className="text-lg font-semibold text-white">Add Income Head</h3>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="form-group flex flex-col gap-1">
                ...
                <input type="text" className="input-field" placeholder="Donations, Hall Rental, etc." value={category} onChange={(e) => setCategory(e.target.value)} disabled={isSubmitting} />
              </div>
        ...
        <div className="col-span-1 lg:col-span-8">
          <div className="glass-card !p-0 overflow-hidden border-emerald-500/10">
            ...
            {isLoading ? (
                  ...
                  <tbody>
                    {heads.map((h) => (
                      <tr key={h.id} className="hover:bg-white/5 transition-colors">
                        <td className="font-semibold text-white">{h.income_category}</td>
              ...
              <div className="p-12 text-center text-zinc-500">No income categories defined yet.</div>
            )}
          </div>
        </div>
      </div>
